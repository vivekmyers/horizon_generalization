...
class TrajectoryUniformSamplingQueue(QueueBase[Sample], Generic[Sample]):
    ...
    def flatten_crl_fn(config, env, transition: Transition, sample_key: PRNGKey) -> Transition:
        ...
        goal = future_obs[:, env.goal_indices]
        future_state = future_obs[:, :env.obs_dim]
...
def train(
    ...
    if process_id == 0 and num_evals > 1:
        ...
        progress_fn(0, metrics, make_policy, None)
    ...
    for _ in range(num_evals_after_init):
        ...
        if process_id == 0:
            ...
            logging.info(metrics)
            progress_fn(current_step, metrics, make_policy, params)
