...
def make_losses(
    ...
    def crl_critic_loss(
        ...
        obs = transitions.extras["state"]
        action = transitions.action
        future_state = transitions.extras["future_state"]
        future_action = transitions.extras["future_action"]
        future_action_rolled = jnp.roll(future_action, 1, axis=0)
        ...
        g_repr = g_encoder.apply(normalizer_params, g_encoder_params, future_state) if energy_fn != "mrn" else jnp.zeros_like(sa_repr)
        if energy_fn == "l2":
    ...
    def actor_loss(
        ...
        goal = jnp.where(random_goal_mask, future_rolled, future_state)

        observation = jnp.concatenate([state, goal[:, env.goal_indices]], axis=1)
        ...
        g_repr = g_encoder.apply(normalizer_params, g_encoder_params, goal) if energy_fn != "mrn" else jnp.zeros_like(sa_repr)
        ...
        if energy_fn == "l2":
            ...
            ga_repr = sa_encoder.apply(
                normalizer_params,
                sa_encoder_params,
                jnp.concatenate([goal, future_action], axis=-1),
            )
