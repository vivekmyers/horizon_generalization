...
def render(inf_fun_factory, params, env, exp_dir, exp_name, log_key):
    ...
    with open(os.path.join(exp_dir, f"{exp_name}.html"), "w") as file:
        file.write(url)
    wandb.log({log_key: wandb.Html(url)})
...
def main(args):
    ...
    if args.use_traj_idx_wrapper:
        raise Exception(
            "traj_index_wrapper_factory does not yet support the new goal indexing scheme."
        )
    ...
    def progress(num_steps, metrics, make_inference_fn, params):
        ...
        try:
            render(make_inference_fn, params, env, run_dir, args.exp_name, "train_render")
            render(make_inference_fn, params, eval_env, run_dir, args.exp_name, "eval_render")
    ...
    render(make_inference_fn, params, env, run_dir, args.exp_name, "final_train_render")
    render(make_inference_fn, params, eval_env, run_dir, args.exp_name, "final_eval_render")
