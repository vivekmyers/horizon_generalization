...
class Ant(PipelineEnv):
    ...
    def reset(self, rng: jax.Array) -> State:
        ...
        # set the target q, qd
        rng, target = self._random_target(rng, 50)
        rng, start = self._random_target(rng, self.goal_dist)
        start = start + target
        q = q.at[-2:].set(target)
        qd = qd.at[-2:].set(0)
        q = q.at[:2].set(start)
        ...
        metrics = {
            ...
            "success_easy": zero,
            "progress": zero,
            "total_distance": jnp.linalg.norm(target),
        }
        info = {"seed": 0, "original_dist": jnp.linalg.norm(target)}
    ...
    def _random_target(self, rng: jax.Array, dist: float) -> Tuple[jax.Array, jax.Array]:
        """Returns a target location in a random circle slightly above xy plane."""
        rng, rng1, rng2 = jax.random.split(rng, 3)
        dist = self.goal_dist * jax.random.uniform(rng1)
